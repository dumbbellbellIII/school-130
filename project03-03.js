...
//Declare the hmlcode variable, setting its initial value to empty 
  text string 
  let htmlCode = "";
//the following arrays have same length
let titles = ["Title1". "Title2", "Title3"];
let links  = ["link1.html", "link2.html", "link3.html"];
let summaries = ["Summary1", "Summary2", "Summary3"];
let ratings = ["5/5", "4/5", "3/5"];

//Create loop counter
for (let i = 0; i < titles.length; i++){
      htmlCode += '<tr>
      <td><a href='${link[i]>${itles[i]}</a></td>
      <td>${summaries[i]</td>
      <td>${ratings[i]}</td>
      </tr>
